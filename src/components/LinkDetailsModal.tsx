...
export default function LinkDetailsModal({ item, profile, onClose }: LinkDetailsModalProps) {
  ...
  const getButtonText = () => {
    if (isEmail) return 'Kirim via Gmail';
    if (isWhatsapp) return 'Kirim WhatsApp';
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className={`relative w-full ${isProjects ? 'max-w-md' : 'max-w-sm'} rounded-[24px] bg-gradient-to-b from-[#16191f] to-[#0e1013] border-2 border-[#242933] overflow-hidden shadow-[0_25px_60px_-15px_rgba(0,0,0,0.9)] p-6 z-10 flex flex-col max-h-[85vh]`}
        >
          ...
          {!isProjects ? (
            ...
            <>
              {/* Projects Scrollable Showcase Box */}
                      ...
                      <div className="flex items-center space-x-3 min-w-0 mr-3">
                        ...
                        <div className="text-left min-w-0">
                          <div className="flex items-center space-x-1.5 flex-wrap">
                            ...
                            <span className="text-[8px] bg-amber-500/10 text-amber-400 font-mono tracking-wider px-1.5 py-0.5 rounded border border-amber-500/10 font-bold uppercase scale-90 origin-left">
                              {proj.tag}
                            </span>
                          </div>
